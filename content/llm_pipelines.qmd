...
3. Produce an output – optionally with internal loops, but without LLM-driven control flow.
...
No tool selection is delegated to the LLM; it only writes the report. This is already powerful – but the __quality__ of the report still varies, and we need a way to __evaluate__ it.
...
3. Informally evaluate the story: What works well? What doesn’t? How did you decid
4. Now ask the model to generate __50–100__ such stories (use a higher temperature).
...
4. Reading and judging everything manually is too slow and doesn’t scale. (We don't have time for this.)
...
1. __Generator:__ produces a candidate output (text, code, EDA report, answer, …).
...
<!-- TODO:give the anthropic harmless finetuning example here (imgs/CAI_illustration.png)-->
...
3. Send a good system prompt and the relevant information to an LLM. 
4. Let it generate a hypothesis (or several). You may want to increase the model's temperature to get some variation. 
...
The example above is still essentially a pipeline: the order of steps is fixed (Generator → Reviewer → Editor …). We could, however, introduce an orchestrator agent that decides which component to call next^[Note that this is, of course, only one way to implement this as an agent system.]:
