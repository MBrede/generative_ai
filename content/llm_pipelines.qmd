...
2. Implement the `hard_check` function above (or extend it with your own criteria). No wrapper class is needed — pass it directly in the edge tuple.
3. Wire as a `Workflow`: `("START", advisor, hard_check, {"pass": ...})`. Task 4 fills in the target of the `"pass"` edge.
4. Test it: what happens when you pass a minimal spec that produces a short output?
