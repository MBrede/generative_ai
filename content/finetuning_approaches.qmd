...
@tip-softPromptFineTuningResources illustrates the amount of resources that can be saved when using soft-prompting-based finetuning compared to full finetuning.

:::{#tip-softPromptFineTuningResources .callout-tip collapse="true" appearance="minimal"}

{{< include resources_softprompt_finetuning.md >}}
...
@tip-LoRAFineTuningResources illustrates the amount of resources that can be saved when using LoRA-based finetuning compared to full finetuning.

:::{#tip-LoRAFineTuningResources .callout-tip collapse="true" appearance="minimal"}

{{< include resources_lora_finetuning.md >}}
...
![Illustration of the result of quantization to 32, 16, 8 and 4 bits. The top of the image shows the same color-gradient under all quantizations, the bottom image is the quantized chapter-illustration.](/img/quantization.png){#fig-quantization .enlarge-onhover}
...
Use the (IA)³ implementation in the huggingface PEFT-module to finetune a small QWEN-model.

* Use the 1.8B version of [QWEN](https://huggingface.co/Qwen/Qwen1.5-1.8B) to base your finetuning on
