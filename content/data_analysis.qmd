...
<!-- TODO: Demo: LLM erhält kleine Tabelle + Frage („Which countries generate the highest revenue?“) → generiert sowohl pandas-Code als auch verbale Antwort.

Problematisierung: „Wie weit darf man dem trauen? Wo sind Risiken?“ (Stichworte: Halluzinationen, falscher Code, Sicherheitsrisiken). -->
...
In a scientific (knowledge gathering) context, you ideally deal with hypotheses. In the commercial world, you usually jump straight to (high-level) questions instead. 

> *“On what device do users have the highest return rate?.”*

Those high-level questions can then be broken down into several low-level questions. In this lecture, we tried to stick to proper hypotheses, but the line got blurred several times (as it would be in the real world). 
...
> This suggests that mobile purchasing behavior is more exploratory or less informed.”*

As the example shows, an insight is *verified* or *falsified* *hypothesis*, together with an *interpretation*. 
...
- The hypothesis generator takes a description of the dataset (and other relevant information) and proposes a high-level hypotheses. It then breaks down the hypothesis into low-level questions.
...
- _“Which customer segments contribute most to total revenue?”_
- _“How do return rates differ by country and device type?”_
- _“Are new customers more likely to return items than returning customers?”_
- _“Did revenue patterns change over the last 6 months?”_
...
- Q1: _“What is the total revenue per country?”_
- Q2: _“What is the total revenue per device_type?”_
- Q3: _“What is the average revenue per order for new vs. returning customers?”_
- Q4: _“How many unique customers are active in each country?”_

For the hypothesis:

> _“Mobile users have a higher return rate than desktop users.”_

we might ask:

- Q5: _“What is the overall return rate per device_type?”_
- Q6: _“How has the return rate for mobile vs. desktop changed over time?”_
...
As shown above, the hypothesis generator has two tasks. 

1. Produce a hypothesis or a  high-level question regarding the data set. 
2. Break down the hypothesis or high-level question into several low-level questions. 
...
- additional information or context, maybe a user goal.
...
2. Implement the hypothesis generator function. 
3. Test the output. Does it generate proper hypotheses?
4. Implement a function ``generate_low_level_questions`` similar to the example above.
5. Inspect the generated questions:
    - Are they answerable with the available columns?
    - Do they match the high-level question?

**Goal**: Practice the distinction between high-level hypothesis and low-level questions and build a first pipeline component.
:::
...
## 📝 Task: From Low-Level Answers to Insights
...
**Note:** This is where state-of-the-art models struggle (at least in 2025), see [@perezLLMBasedApproachInsight2025] and [@baiInsightAgentsLLMBased2025]. Don't be too frustrated if it does not work with small open-source models. It is, however, good for educational purposes to experience the limits of LLMs. 
...
## 📝 (optional Task) – Designing a Simple Verifier
