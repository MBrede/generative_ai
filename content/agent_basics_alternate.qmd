...
Before we get into agents proper, let us first discuss prompting strategies for solving complex problems. Below are two examples for using custom prompts that help the LLM in answering more complex problems by first dividing the problem into smaller steps before solving each step separately. 
