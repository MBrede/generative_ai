...
The raining position of GANs as the de-facto standard for AI image generation was challenged by the release of DALL-E by OpenAI in January 2021.
...
<!-- ![Illustration of the DALL-E training process.](../imgs/dalle_architecture.png){#fig-dVAE .enlarge-onhover} -->
...
This is done by training both a vision- and a text-transformer as encoders to maximize the cosine similarity between the encoded image and text for each pair in the matrix and minimizing it for all other pairs.
...
The diffusion model's central principle is the training on a sequence of gradually noised images. This is realized by systematically adding noise to an image over a series of steps, transforming the original image into pure noise. At each step, the diffusion model is then trained to reverse this process, effectively learning to denoise or reconstruct towards the original image from its noised versions^[Actually, the model is learning to predict the _noise_ in the image, not the _denoised image_ itself. This is due to some Bayes magic (namely the actually so-called re-parameterization trick [@kingmaAutoencodingVariationalBayes2013]). ]. This is illustrated in @fig-diffusion.
...
1. They introduce a method called latent diffusion, which allows them to generate high-resolution images more efficiently by operating on a lower-dimensional representation of the image data. This is achieved by using an autoencoder (VAE) to compress the original image into a smaller latent space and then applying the diffusion process to this compressed representation. This process is built on work by @esserTamingTransformersHighResolution2021 and is conceptually similar to the dVAE-approach utilized by DALL-E 
2. They use a denoising diffusion probabilistic model (DDPM) as the fundamental generation-process for their architecture, which allows them to generate high-quality images with fewer steps compared to previous methods. This DDPM-model is implemented as a time-conditional UNet. 
2. To improve the quality of generated images and reduce artifacts, they additionally introduce a technique called classifier guidance. This involves using a pretrained text-to-image model (like CLIP) to guide the diffusion process towards generating images that align better with the input text prompt. This is implemented by augmenting the UNet-architecture with a cross-attention-mechanism that allows the denoising to consider additional multimodel embeddings in the denoising-process (see @fig-ldm for an illustration of the architecture).
...
This architecture has been widely adopted and is used as a foundation for many state-of-the-art text-to-image models, including Stable Diffusion, as well as DALL-E 2.
...
2. Generate some images using different prompts and seeds. What do you observe?
...
Use [this]() colab notebook to test out Unidiffuser using the huggingface `diffusers`-module and generate some images and text.

Try the tips you tested on the basic SD-model and test whether the model accurately generates descriptions for you generated images.
